...
VER=10.4
VERHUMAN=$VER
PKG=omniti/database/postgresql-104/citext
DOWNLOADDIR=postgres
MODULE=citext
CONTRIBDIR=contrib/$MODULE
SUMMARY="$PROG $MODULE - Case Insensitive Text for PostgreSQL $VER"
DESC="$SUMMARY"
...
if [[ "$DLPX51_CITEXT_PREFIX" ]]; then
    PREFIX=$DLPX51_CITEXT_PREFIX
fi

if [[ "$DLPX51_CITEXT_PKG" ]]; then
    PKG=$DLPX51_CITEXT_PKG
fi

unset DLPX51_CITEXT_PKG
unset DLPX51_CITEXT_PREFIX
